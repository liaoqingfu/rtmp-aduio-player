...
void AlsaPcm::setAudioParams(int &fmt, unsigned int &samplerate, unsigned short &channels)
{
    ...
    snd_pcm_hw_params_malloc(&pcm_params_);                     //·ÖÅäparams½á¹¹Ìå
    ret = snd_pcm_hw_params_any(pcm_handle_, pcm_params_);      //³õÊ¼»¯params
    if (ret < 0)
    {
    ...
    ret = snd_pcm_hw_params_set_access(pcm_handle_, pcm_params_, SND_PCM_ACCESS_RW_INTERLEAVED);    //³õÊ¼»¯·ÃÎÊÈ¨ÏÞ
    if (ret < 0)
    {
    ...
    ret = snd_pcm_hw_params_set_format(pcm_handle_, pcm_params_, (snd_pcm_format_t)fmt);        //ÉèÖÃ16Î»²ÉÑù¾«¶È 
    if (ret < 0)
    {
    ...
    ret = snd_pcm_hw_params_set_channels(pcm_handle_, pcm_params_, channels);                   //ÉèÖÃÉùµÀ,1±íÊ¾µ¥Éù>µÀ£¬2±íÊ¾Á¢ÌåÉù        
    if (ret < 0)
    {
    ...
    ret = snd_pcm_hw_params_set_rate_near(pcm_handle_, pcm_params_, &samplerate, &dir);     //ÉèÖÃ>ÆµÂÊ
    if (ret < 0)
    {
